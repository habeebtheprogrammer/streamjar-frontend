...
class Join extends Component {
    ...
    subscribe(e) {
        e.preventDefault();
        // if(!this.state.tos) return this.setState({error:"Please agree to our tos"})
                            ...
                            <div className="row join-bg white-text" style={{ margin: "0px" }}>
                                <div className="col s12" style={{ minHeight: "400px", background: "", padding: "20px" }}>
                                    ...
                                    <form onSubmit={this.subscribe} className="row" style={{paddingTop:"20px"}}>
                                        ...
                                        <Col s={12}>
                                            <label>
                                                <div>  <input required style={{  opacity: "1", left: "0", width: "inherit", position: "relative" }} name="tos" onChange={() => this.setState({ tos: !this.state.tos  })} type="checkbox" className="input" title="" /> By singing up, I agree to recieve commercial information from TamTamTools located in Louvain-La-Neuve, Place de L’Université 16, Belgium (Data Administrator). I can withdraw my consent at any time. The data will be processed until the consent is withdrawn.</div>

                                                <div>The administrator processes data following the <a href="privacy-policy" >privacy policy</a>. I have the right to access, rectify, delete or limit processing of the data, the right to object, the right to submit a complaint to the supervisory authority or transfer the data.</div>
