...
import classnames from "classnames"
 class Comments extends Component{
    ...
    postComment(post) {
        ...
        axios.post(`${apiUrl}/api/postForumComment`, { "description": this.state.description,creatorID:post.userID, userID:this.props.auth.user.id, postID:post._id}).then((res) => {
            if (res.data.success) {
    ...
    postReplyComment(postID,commentID) {
       ...
       { axios.post(`${apiUrl}/api/postReplyComment`, { "text": this.state.reply.text,postID:postID, userID:this.props.auth.user.id, commentID: commentID}).then((res) => {
            if (res.data.success) {
               window.location.reload();
            }
        }).catch((err)=>this.setState({error:"An error has occured"}))
    ...
    likeComment(post,comment) {
        let token = localStorage.getItem("kaytoken")
        axios.post(`${apiUrl}/api/likeForumComment`, { creatorID:post.userID, userID:this.props.auth.user.id, postID:post._id,commentID:comment._id}).then((res) => {
            if (res.data.success) {
               window.location.reload();
            }
        }).catch((err)=>this.setState({error:"An error has occured"}))
    ...
    unLikeComment(post,comment) {
        let token = localStorage.getItem("kaytoken")
        axios.post(`${apiUrl}/api/unLikeForumComment`, { userID:this.props.auth.user.id, postID:post._id,commentID:comment._id}).then((res) => {
            if (res.data.success) {
               window.location.reload();
            }
        }).catch((err)=>this.setState({error:"An error has occured"}))
